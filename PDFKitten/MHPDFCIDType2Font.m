...
- (void)setCIDToGIDMapWithDictionary:(CGPDFDictionaryRef)dict
{
	...
	else if (type == kCGPDFObjectTypeStream)
	{
		...
		MHLog(@"CIDType2Font: no implementation for CID mapping with stream (%d bytes)", [data length]);
		[data release];
	}
}
...
//			unichar unicodeValue = 0x4ea4;  
			MHLog(@"%C %x", unicodeValue, unicodeValue);
		}
